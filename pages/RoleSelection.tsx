...
const RoleSelection: React.FC = () => {
  ...
  const handleContinue = () => {
    ...
    if (!storedUser || !selectedRole) return;
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6">
      ...
      <div className="text-center mb-16 max-w-md">
        ...
        <p className="text-slate-400 font-medium">
          Choose your workspace to continue with specialized tools for your daily tasks.
        </p>
      </div>
      ...
      <button
        ...
        className="px-16 py-4 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold shadow-xl shadow-blue-200 transition-all active:scale-[0.98] disabled:opacity-50"
      >
        Continue
      </button>
    </div>
...
const RoleCard: React.FC<RoleCardProps> = ({
  ...
  <button
    onClick={onClick}
    className={`flex-1 text-left p-10 rounded-[2rem] border-4 transition-all duration-300 relative group ${
      isSelected
  ...
  >
    ...
    <div
      className={`w-16 h-16 rounded-2xl flex items-center justify-center text-2xl mb-8 transition-colors ${
        isSelected
    ...
    <h3
      className={`text-2xl font-extrabold mb-4 transition-colors ${
